...
if ! command -v nix &>/dev/null; then
    ...
    echo "WARN: \$ curl --proto '=https' --tlsv1.2 -sSf -L https://install.lix.systems/lix | sh -s -- install" >&2
    echo 'WARN: --------------------------------------------------------------------------' >&2
    totalCount=3
    for count in $(seq $totalCount); do
        ...
        read -r -p "> " res
        if [[ "$res" == 'y' || "$res" == 'Y' ]]; then
            if curl --proto '=https' --tlsv1.2 -sSf -L https://install.lix.systems/lix | sh -s -- install; then
